...
void RenderScene()
{
	...
	gpLightingShader->SetValue("gSpecularPower", &gSpecularPower, sizeof(gSpecularPower));
...
bool LoadAssets()
{
	...
	// 쉐이더 로딩
	gpLightingShader = LoadShader("Resources/Shader/Lighting.fx");
	if (!gpLightingShader) return false;
	// 모델 로딩
