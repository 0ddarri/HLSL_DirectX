...
void RenderScene()
{
	...
	gpUVAnimationShader->SetVector("gWorldLightColor", &gLightColor);
	gpUVAnimationShader->SetTexture("DiffuseMap_Tex", stoneDiffuse);
	gpUVAnimationShader->SetTexture("SpecularMap_Tex", stoneSpecular);
...
bool LoadAssets()
{
	...
	// 쉐이더 로딩
	gpUVAnimationShader = LoadShader("Resources/Shader/UVAnimation.fx");
	if (!gpUVAnimationShader) return false;
	// 모델 로딩
	gpTorus = LoadModel("Resources/Model/Torus.x");
	if (!gpTorus) return false;
	return true;
}
