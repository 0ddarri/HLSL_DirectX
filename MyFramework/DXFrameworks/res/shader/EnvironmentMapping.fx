...
float4 EnvironmentMapping_Pass_0_Pixel_Shader_ps_main(PS_INPUT Input) : COLOR
{
   ...
   tangentNormal = lerp(float3(0.5,0.5,1), normalize(tangentNormal * 2 - 1), gNormalStrength);
   ...
   float3 environment = texCUBE(EnvironmentSampler, viewReflect).rgb * gLightColor.rgb;
   
   return float4(ambient + specular + diffuse + environment * 0.5f, 1);
}
//--------------------------------------------------------------//
