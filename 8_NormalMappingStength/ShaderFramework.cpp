...
void RenderScene()
{
	...
	D3DXVECTOR3 vEyePos(gWorldCameraPosition); // 카메라 위치
	...
	gpSpecularShader->SetMatrix("gWorldMatrix", &matWorld);
	gpSpecularShader->SetMatrix("gWorldViewProjectionMatrix", &matWorldViewProj);
	gpSpecularShader->SetVector("gWorldCameraPosition", &gWorldCameraPosition);
	gpSpecularShader->SetVector("gWorldLightPosition", &gWorldLightPosition);
	gpSpecularShader->SetVector("gWorldLightColor", &gLightColor);
	gpSpecularShader->SetTexture("DiffuseMap_Tex", stoneDiffuse);
	gpSpecularShader->SetTexture("SpecularMap_Tex", stoneSpecular);
	gpSpecularShader->SetTexture("NormalMap_Tex", stoneNormal);
	gpSpecularShader->SetFloat("gNormalStrength", gNormalStrength);

	gNormalStrength += 0.01f;
	if (gNormalStrength > 5.0f) gNormalStrength = 0.0f;

	UINT numPasses = 0;
	gpSpecularShader->Begin(&numPasses, NULL);
	for (UINT i = 0; i < numPasses; i++)
	{
		gpSpecularShader->BeginPass(i);
		gpSphere->DrawSubset(0);
		gpSpecularShader->EndPass();
	}
	gpSpecularShader->End();
}
...
bool LoadAssets()
{
	...
	// 쉐이더 로딩
	gpSpecularShader = LoadShader("Resources/Shader/normalMapping_Strength.fx");
	if (!gpSpecularShader) return false;
	// 모델 로딩
	gpSphere = LoadModel("Resources/Model/SphereWithTangent.x");
...
void Cleanup()
{
	...
	gpSphere = NULL;
	// 쉐이더를 release 한다.
	gpSpecularShader->Release();
	gpSpecularShader = NULL;
	// 텍스처를 release 한다.
