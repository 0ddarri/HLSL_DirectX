...
void RenderScene()
{
	...
	D3DXMATRIXA16 invMatWorld;
	D3DXMatrixTranspose(&invMatWorld, &matWorld);
	...
	gpToonShader->SetMatrix("gWorldViewProjectionMatrix", &matWorldViewProj);
	gpToonShader->SetMatrix("gInvWorldMatrix", &invMatWorld);
...
bool LoadAssets()
{
	// 텍스처 로딩
	// 쉐이더 로딩
	gpToonShader = LoadShader("Resources/Shader/toonShading.fx");
	if (!gpToonShader) return false;
	// 모델 로딩
	gpTeapot = LoadModel("Resources/Model/teapot.x");
...
LPD3DXEFFECT LoadShader(const char * filename)
{
	...
	if (!ret && pError)
	{
		std::wcout << "load Shader Fail " << filename << std::endl;
...
LPD3DXMESH LoadModel(const char * filename)
{
	...
	if (FAILED(D3DXLoadMeshFromX(filename, D3DXMESH_SYSTEMMEM, gpD3DDevice, NULL, NULL, NULL, NULL, &ret)))
	{
		...
		OutputDebugString("\n");
		std::wcout << "load Model FAil " << filename << std::endl;
	};
	std::wcout << "load Model " << filename << std::endl;
	return ret;
}
